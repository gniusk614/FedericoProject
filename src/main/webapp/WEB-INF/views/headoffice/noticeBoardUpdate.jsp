				...
				<div class="card" id="contentCard">
					<div class="card-header">
						<div class="form-group mb-2">
							<input type="text" id="title" placeholder="제목" value="${noticeDetail.title}"
								class="form-control">
							<input type="text" id="seq" placeholder="글번호" value="${noticeDetail.seq}" hidden="hidden"
								class="form-control">
						</div>
					</div>
					<div class="card-body">
						...
						<div class="row">
							...
							<c:if test="${noticeDetail.noticeFlag=='Y'}">
								<input class="form-check-input me-1" id=noticeFlag type="checkbox" checked="checked"
									aria-label="..."> 상단 고정
							</c:if>
							<c:if test="${noticeDetail.noticeFlag=='N'}">
								<input class="form-check-input me-1" id=noticeFlag type="checkbox"
									aria-label="..."> 상단 고정
							</c:if>
							</div>
							<div class="col" align="left">
								<button class="btn btn-primary" onclick="boardUpdate()">수정</button>
								<button class="btn btn-primary" onclick="location.href='noticeBoardf'">취소</button>
							</div>
						</div>
					</div>
					</div>
